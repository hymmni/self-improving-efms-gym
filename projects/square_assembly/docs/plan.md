...
```
square_assembly/
├── pyproject.toml            # 패키지: square_assembly, optional-deps: train/eval
...
### 시뮬레이터 의존 체인 (참고)
`square_assembly → robomimic(데이터셋·env wrapper) → robosuite(태스크·로봇) → MuJoCo(물리엔진)`
...
**50 epoch 실제 학습 실행 → rollout 평가 success_rate=0.9 (20 에피소드 중 18개 성공).** 데이터→모델→학습→rollout 전체 파이프라인이 실제로 동작함을 확인. wandb 로깅 연결(`leeju0917` 계정, project=`square_assembly`) — run: https://wandb.ai/leeju0917-seoul-national-university-ofscience-and-technology/square_assembly/runs/chz119qv . Loss는 1.13(step 0) → 0.05 근처(epoch 10)로 빠르게 감소 후 0.03~0.07 사이 평탄화.
...
**구현 위치**:
- `src/square_assembly/networks/conditional_unet1d.py` — 1D conditional U-Net (FiLM), Chi et al./LeRobot의 공개된 표준 아키텍처를 구조 참고해 자체 구현(flare 코드 import 없음).
- `src/square_assembly/policies/diffusion/diffusion_policy.py` — `DiffusionPolicyLowDim`. low_dim obs만 지원(vision encoder는 M3). obs_horizon(To) 프레임을 이어붙여 global conditioning 벡터로 사용. 학습 noise scheduler=DDPM, 추론 sampling=DDIM (diffusers).
- `src/square_assembly/envs/robomimic/factory.py` — `make_lowdim_env()`. robomimic import는 이 파일과 datasets 어댑터에만 존재(원칙 유지).
- `src/square_assembly/runners/rollout.py` — receding horizon(Ta<=Tp) closed-loop rollout. `env.is_success()['task']`로 성공 판정.
- `src/square_assembly/configs/{train,eval}.yaml` + `configs/{task,policy}/` — hydra 설정 분리. task=lift_low_dim, policy=diffusion_unet_lowdim.
- `src/square_assembly/scripts/{train,eval}.py` — hydra 진입점.
...
**conda 환경**: `square_assembly` (python 3.10). `requirements.txt`가 버전 단일 출처, `environment.yml`은 그걸 참조.
...
- 조치: `.bashrc`의 ROS2 source 2줄만 주석 처리(삭제 아님, 백업 `~/.bashrc.bak_20260703_092935`) + `square_assembly` conda 환경 전용 activate.d/deactivate.d 훅으로 `PYTHONNOUSERSITE=1` 설정·PYTHONPATH 격리. 다른 conda 환경(rise, openvla 등)엔 영향 없음.
...
**구현 위치**: `src/square_assembly/datasets/normalization.py`(MinMax 통계·정규화), `src/square_assembly/datasets/robomimic_dataset.py`(robomimic 격리 어댑터). robomimic import는 이 두 파일에만 존재.
...
**처리됨** — `src/square_assembly/factory.py`(registry: policy는 빌더 함수, runner는 클래스),
