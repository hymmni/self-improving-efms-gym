...
> `self-improving-gym`으로 통합된, 축소된 서브셋이다** (`.git` 제거, ADR-007 참고).
...
> `src/square_assembly/factory.py`·`envs/robomimic/factory.py` 상단의 "통합 시 축소" 주석 참고.
>
> Diffusion Policy(원본 그대로) + robomimic 인프라 위에 SI-EFM식 DDPO를 얹는 구현은
> `self-improving-gym`의 `phases/`에서 설계·진행한다(이 디렉토리 자체는 원본
...
**스택 레이어** (아래로 갈수록 구체적):
```
square_assembly (이 repo: Diffusion Policy, 학습 루프)
   ↓
...
### 학습
```bash
python -m square_assembly.scripts.train                              # configs/train.yaml 기본값(square, diffusion, image)
python -m square_assembly.scripts.train task=square_stage             # stage conditioning 켜기
python -m square_assembly.scripts.train task=square_low_dim policy=diffusion_unet_lowdim policy_name=diffusion_lowdim  # 구 low_dim 경로
python -m square_assembly.scripts.train policy=openvla policy_name=openvla runner_name=openvla_trainer batch_size=2 max_steps=3000
python -m square_assembly.scripts.train resume=true                   # 같은 output_dir의 최신 체크포인트/adapter에서 이어받기
```
...
```bash
python -m square_assembly.scripts.eval checkpoint_path=outputs/train/square_diffusion/policy_epoch300.pt num_episodes=50
python -m square_assembly.scripts.eval task=square_stage checkpoint_path=... num_episodes=50   # stage 체크포인트(온라인 stage tracker 자동)
python -m square_assembly.scripts.eval task=square policy_name=openvla policy=openvla \
...
unset MUJOCO_GL
python -m square_assembly.scripts.eval task=square_low_dim checkpoint_path=... num_episodes=3 render=true

# image: 오프스크린(egl) 렌더 + OpenCV 창(mjviewer 온스크린과 동시 사용 시 GL 충돌로 세그폴트 — 실측 지뢰)
MUJOCO_GL=egl DISPLAY=:1 python -m square_assembly.scripts.eval checkpoint_path=... num_episodes=3 render=true
```
...
python -m square_assembly.scripts.collect checkpoint_path=outputs/train/.../policy_epochN.pt

# 키보드 개입, policy_name으로 BC/DiffusionPolicy(low_dim·image) 아무거나 배포 가능
python -m square_assembly.scripts.collect policy_name=bc_rnn_lowdim intervention_device=keyboard
```
...
```bash
python -m square_assembly.scripts.train task.hdf5_path=outputs/intervention/round.hdf5 weighting.kind=class_based
```
...
```bash
python -m square_assembly.scripts.round task=door_cabinet_low_dim policy=diffusion_unet_lowdim \
...
### 진단 도구
```bash
python -m square_assembly.scripts.label_stages --hdf5 data/robomimic/square/ph/v1.5/square/ph/square_image_v15.hdf5
python -m square_assembly.scripts.stage_counterfactual --checkpoint outputs/train/square_stage_diffusion/policy_epoch300.pt
```
...
```bash
python -m square_assembly.scripts.train task=can_low_dim num_epochs=50
python -m square_assembly.scripts.eval task=can_low_dim \
