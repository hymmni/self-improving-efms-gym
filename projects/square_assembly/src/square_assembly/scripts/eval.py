...
from square_assembly.runners.rollout import maybe_speed_up_inference, rollout_policy, rollout_policy_async
from square_assembly.utils.checkpoints import load_run_config
from square_assembly.utils.task_utils import is_image_task, is_piper_task, is_robocasa_task, make_eval_env, task_obs_keys
...
def _patch_mjviewer_esc_quit():
    ...
    확인). site-packages는 안 건드리고 square_assembly 쪽에서 update()만 ESC 콜백 추가해 패치한다.
...
def _run_dp_eval(cfg, device):
    ...
    if cfg.task.get("use_online_stage_tracker", False):
        from square_assembly.datasets.stage_labeler import (
...
def _run_openvla_eval(cfg):
    ...
    from square_assembly.datasets.openvla_dataset import SQUARE_INSTRUCTION
