...
from square_assembly.datasets.apo_sampler import build_balanced_sampler, build_interaction_exhaustion_sampler
from square_assembly.datasets.normalization import (
...
from square_assembly.datasets.zarr_dataset import ZarrSequenceDataset
from square_assembly.factory import registry
from square_assembly.losses.sirius_loss import sirius_loss
from square_assembly.runners.rollout import rollout_policy
from square_assembly.utils.checkpoints import (
    get_latest_epoch_checkpoint, load_resume_state, save_epoch_checkpoint, save_resume_state, save_run_config,
)
from square_assembly.utils.task_utils import (
...
class DiffusionTrainer:
    def __init__(self, cfg, policy, device):
        ...
        if weighting_kind == "class_based":
            from square_assembly.weighting.class_based import ClassBasedWeight
            ...
            from square_assembly.weighting.action_error import ActionErrorWeight
            ...
            from square_assembly.weighting.rabc import RABCWeight
            ...
            from square_assembly.weighting.phase_rule import PhaseRuleWeight
            ...
            from square_assembly.weighting.action_variance import ActionVarianceWeight
            ...
            from square_assembly.weighting.event_radius import EventRadiusWeight
        ...
        if self.uses_zarr:
            ...
            cache_mode = "all" if cfg.num_workers >= 1 else "low_dim"  # h5py fork 크래시 회피(지뢰, square_assembly_status.md)
        ...
        if cfg.task.get("use_online_stage_tracker", False):
            from square_assembly.datasets.stage_labeler import make_online_tracker
            self._stage_tracker = make_online_tracker(cfg.task.name)

    def _stage_extra_obs_fn(self, obs_raw):
        from square_assembly.datasets.stage_labeler import num_stages_for_task, onehot as stage_onehot
        import numpy as np
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if (epoch + 1) % self.cfg.ckpt_every_epochs == 0 or is_last:
                ...
                if self.system is not None and getattr(self.system, "use_lora", False):
                    import copy
                    from square_assembly.networks.lora import merge_lora
