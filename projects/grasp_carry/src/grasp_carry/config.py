...
class CarryConfig:
  ...
  # 캔 높이 규격(사용자 결정으로 상한 완화). 원래 100~140mm(종횡비 1.5~2.3)는
  # `experiments/2026-07-31_grasp-carry-si-rebuild.md`에서 실측된 문제의
  # 원인이었다: 놓친 블록이 공중에서 대부분 옆으로 누워 타겟 밖에서는 영구
  # 실패로, 안에서는 자세 무관 성공으로 빠졌다. 그래서 "놓쳤지만 재시도해서
  # 성공"이라는 성공 쪽 꼬리(조건 B에 필요)가 리드 200에서도 3.3%뿐이었다.
  # 100~115mm(종횡비 1.5~1.9)로 상한을 낮춰 넘어질 확률을 줄인다 — 박스 벽
  # 높이(95mm)는 그대로 둔다. 벽을 올리는 쪽은 반대 방향이다: 노출(= 접촉
  # 길이의 재료)이 `블록높이 - 벽높이`이므로 벽을 올리면 방금 고친 얕은/깊은
  # 파지 그라디언트가 다시 무너진다.
  object_height_range: Tuple[float, float] = (100.0, 115.0)  # mm; 캔 높이 규격
  # 은닉 — 빈 캔(~30g) ~ 그립력이 원리적으로 버틸 수 있는 상한 근처(사용자
  # 결정으로 상향). 하드 한계는 `max_hold_mass(mu)` = `2*mu*grip_force/gravity`
  # 다 — 그립력(12N)이 마찰만으로 무게를 지탱하므로, 이 한계를 넘는 질량은
  # 어떤 파지 기술로도(속도를 아무리 줄여도) 들리지 않는다. 최악 마찰
  # (mu=0.25)에서 한계가 611.6g이라, 500g은 그 아래 1.22배 여유를 남긴다.
  # 참고로 이 캔 크기(지름 60~70mm, 높이 100~115mm) 원통을 철(7.85g/cc)로
  # 채우면 약 2.8kg으로, 최선 마찰(mu=0.65, 한계 1590g)에서도 1.8배를
  # 넘는다 — "쇳덩이" 밀도는 이 그립력으로 원리적으로 들 수 없다(임의
  # 상수가 아니라 `max_hold_mass`에서 유도된 하드 제약).
  object_mass_range: Tuple[float, float] = (0.030, 0.500)    # kg
  ...
  # 소스 박스 벽 높이. **박스에 못 들어가는 에피소드의 접촉 길이를 정하는 값**이다.
  # 그리퍼가 박스에 못 들어가면 손가락 끝이 rim까지만 내려가므로 패드 구간은
  # [rim - pad_length, rim]으로 고정된다. 따라서
  #
  #     접촉 길이 = min(pad_length, 블록 높이 - 벽 높이)      ... (*)
  #
  # 즉 **rim 위로 노출된 길이**가 곧 접촉 길이이고, 그것이 패드 길이(30mm)를
  # 넘는 순간 접촉이 포화해 얕은/깊은 파지가 구분되지 않는다.
  #
  # 이전 값 75mm는 위 식을 블록 높이 범위의 **한쪽 끝(100mm)에서만** 확인해
  # "노출 25mm < 패드 30mm"라고 유도했으나, 실제 블록은 100~140mm라 노출이
  # 25~65mm가 되어 12개 시드 전부 접촉 길이가 30.0mm로 포화했다(실측).
  # 파지 깊이를 가르는 것이 설계 의도인 박스 내폭이 아니라 블록 높이가 되어
  # 버렸고, 좁은 박스가 난이도를 만들지 못했다.
  #
  # 95mm는 노출을 5~45mm로 만들어 (*)의 접촉 길이가 5~30mm에 **걸치게** 한다.
  # 낮은 블록일수록 윗동아리만 얕게 물려 회전 저항(mu*N*접촉길이)이 작고
  # 지렛대는 길다 = 아슬아슬한 파지 → 재파지의 동기가 생긴다. 높은 블록은
  # 패드가 포화해 안정적으로 물린다.
  ...
  #  눕는다. 보장되는 것은 **스폰 상태의 안정성**이다. 박스 안에서 넘어진
